...
export default {
  ...
  post: {
    "/generations": async (request: Request) => {
      ...
      let imageUrls = [];
      try {
        imageUrls = await generateImages(finalModel, prompt, {
          ratio,
          resolution,
          sampleStrength,
          negativePrompt,
        ...
        markTaskGenerated(taskId, imageUrls);
      } catch (err) {
    ...
    "/compositions": async (request: Request) => {
      ...
      if (isMultiPart) {
        ...
        materials = materialsFromFiles({ images: imageFiles });
        ...
        materials = materialsFromPaths(images, "image");
      }
      ...
      let resultUrls = [];
      try {
        resultUrls = await generateImageComposition(finalModel, prompt, images, {
          ratio,
          resolution,
        ...
        markTaskGenerated(taskId, resultUrls);
      } catch (err) {
        const message = errorMessage(err);
        if (taskId) markTaskGenerationFailed(taskId, message);
        else createFailedSubmitTask(taskContext, message);
