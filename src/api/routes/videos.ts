...
import {
    buildTaskContext,
    createFailedSubmitTask,
    createSubmittedTask,
    errorMessage,
    markTaskGenerated,
    markTaskGenerationFailed,
    materialsFromBodyFields,
    materialsFromFiles,
    materialsFromPaths,
    mergeMaterials,
} from '@/admin/task-recorder.ts';
...
export default {
    ...
    post: {
        ...
        '/generations': async (request: Request) => {
            ...
            const materialFieldNames = ['image_file', 'video_file'];
            for (let i = 1; i <= 9; i++) materialFieldNames.push(`image_file_${i}`);
            for (let i = 1; i <= 3; i++) materialFieldNames.push(`video_file_${i}`);
            const materials = mergeMaterials(
                materialsFromPaths(finalFilePaths, 'image'),
                materialsFromFiles(request.files),
                materialsFromBodyFields(request.body, materialFieldNames),
            );
            ...
            try {
                ...
                generatedVideoUrl = await generateVideo(
                    ...
                    prompt,
                    {
                        ...
                        functionMode,
                        onTaskSubmitted: (historyId) => {
                            taskId = createSubmittedTask(taskContext, historyId);
                        },
                    },
                    token
                );
                markTaskGenerated(taskId, [generatedVideoUrl]);
            } catch (err) {
                const message = errorMessage(err);
                if (taskId) markTaskGenerationFailed(taskId, message);
                else createFailedSubmitTask(taskContext, message);
                throw err;
            }
