...
// 伪装headers
const BROWSER_PROXY_URI = "/mweb/v1/aigc_draft/generate";
const FAKE_HEADERS = {
...
export async function request(
  ...
  while (retries <= maxRetries) {
    try {
      ...
      const response = uri === BROWSER_PROXY_URI
        ? await BrowserService.getInstance().request({
          method,
          url: fullUrl,
          params: requestParams,
          headers,
          data: options.data,
          sessionId: tokenWithRegion,
          proxyUrl,
          timeout: Number(options.timeout || 45000),
          referer: String(headers.Referer || origin),
        })
        : await axios.request({
        method,
        url: fullUrl,
