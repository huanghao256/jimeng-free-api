...
import { DEFAULT_ASSISTANT_ID_CN, DEFAULT_ASSISTANT_ID_US, DEFAULT_ASSISTANT_ID_HK, DEFAULT_ASSISTANT_ID_JP, DEFAULT_ASSISTANT_ID_SG, DEFAULT_VIDEO_MODEL, DRAFT_VERSION, DRAFT_VERSION_OMNI, OMNI_BENEFIT_TYPE, OMNI_BENEFIT_TYPE_FAST, OMNI_BENEFIT_TYPE_FAST_YZ, VIDEO_MODEL_MAP, VIDEO_MODEL_MAP_US, VIDEO_MODEL_MAP_ASIA } from "@/api/consts/common.ts";
...
const DRAFT_MIN_VERSION_OMNI = "3.3.9";
...
export async function queryVideoHistoryResult(refreshToken: string, historyId: string) {
  ...
  if (!historyData) {
    return {
      ...
      itemList: [],
      urls: [],
      historyData: { status: 20, item_list: [] },
      isCompleted: false,
      isFailed: false,
    };
  }
  ...
  let urls = itemList.map((item: any) => extractVideoUrl(item)).filter(Boolean);
  if (!urls.length && (historyData.status === 10 || historyData.status === 50)) {
    const fallbackUrls = await Promise.all(itemList.map(async (item: any) => {
      const itemId = item?.item_id
        || item?.id
        || item?.local_item_id
        || item?.common_attr?.id;
      return itemId ? fetchHighQualityVideoUrl(itemId, refreshToken) : null;
    }));
    urls = fallbackUrls.filter(Boolean) as string[];
  }
  return {
    historyId,
    status: historyData.status,
    failCode: historyData.fail_starling_message,
    itemCount: itemList.length,
    finishTime: historyData.task?.finish_time || 0,
    itemList,
    urls,
    historyData,
    isCompleted: historyData.status === 10 || historyData.status === 50,
    isFailed: historyData.status === 30,
  };
}
...
export async function generateVideo(
  ...
  if (isOmniMode) {
    ...
    const isAsiaInternational = regionInfo.isHK || regionInfo.isJP || regionInfo.isSG;
    const inputVideoDuration = Number(totalVideoDuration.toFixed(3));
    const omniExtraVipFunctionKey = isAsiaInternational ? `${model}-${resolution}` : model;
    ...
    const sceneOption = {
      ...
      reportParams: {
        ...
        vipSource: "generate",
        extraVipFunctionKey: omniExtraVipFunctionKey,
    ...
    // 根据模型选择 benefit_type
    const omniBenefitType = is40
      ? (isAsiaInternational ? OMNI_BENEFIT_TYPE_FAST_YZ : OMNI_BENEFIT_TYPE_FAST)
      : OMNI_BENEFIT_TYPE;
    const omniCommerceInfo = {
      amount: actualDuration,
    ...
    requestData = {
      ...
      data: {
        extend: {
          root_model: model,
          m_video_commerce_info: omniCommerceInfo,
          workspace_id: 0,
          m_video_commerce_info_list: [omniCommerceInfo],
        },
        submit_id: submitId,
        ...
        draft_content: JSON.stringify({
          type: "draft",
          id: util.uuid(),
          min_version: DRAFT_MIN_VERSION_OMNI,
  ...
  logger.info(`Video generation task submitted, history_id: ${historyId}`);
  return historyId;
}
